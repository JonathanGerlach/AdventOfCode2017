...
enum puzzleInput = `nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
void main()
{
	...
	auto listOfPassphrases = puzzleInput.splitter("\n");
	size_t count = 0;

	foreach (p; listOfPassphrases)
	{
		if (p.isPassphraseValid) count++;
	}

	writeln("The number of valid passphrases is " ~ count.to!string);
}

@("Examples succeed")
unittest
{
	enum example1 = "abcde fghij";
	enum example2 = "abcde xyz ecdab";
	enum example3 = "a ab abc abd abf abj";
	enum example4 = "iiii oiii ooii oooi oooo";
	enum example5 = "oiii ioii iioi iiio";
	assert(example1.isPassphraseValid == true);
	assert(example2.isPassphraseValid == false);
	assert(example3.isPassphraseValid == true);
	assert(example4.isPassphraseValid == true);
	assert(example5.isPassphraseValid == false);
}

bool isPassphraseValid(string passphrase)
{
	import std.range : array;
	import std.algorithm : splitter, count, any, map;
	
	// Convert into an array of strings
	auto words = passphrase.splitter(" ");

	// We return false if any word contains all the letters of any other word
	bool isAnAnagram(const string a, const string b)
	{
		bool isAnAnagram = true;

		debug writeln("Comparing a: " ~ a.to!string ~ " to b: " ~ b.to!string);

		// Make a new range of all the characters in common
		import std.range : chain;
		auto both = chain(a, b);

		// Then, make sure that we have the same count of that character in each word
		import std.algorithm : each;
		both.each!((l) { if (a.count(l) != b.count(l)) isAnAnagram = false;} );

		debug
		{
			auto maybeNot = isAnAnagram ? "" : "not ";
			debug writeln("a: " ~ a.to!string ~ " is " ~ maybeNot ~ "an anagram of b: " ~ b.to!string);
		}

		return isAnAnagram;
	}

	auto isValid = !words.map!(w => words.count!isAnAnagram(w) == 1).any!(a => a == false);
