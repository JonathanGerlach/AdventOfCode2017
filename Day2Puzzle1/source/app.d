...
2	4	6	8`;
	...
	assert(checksumLine(lines[0]) == 8);
	assert(checksumLine(lines[1]) == 4);
	assert(checksumLine(lines[2]) == 6);
	import std.conv : to;
	debug writeln("Checksum is " ~ checksum(input).to!string);
	assert(checksum(input) == 18);
}

@("We can iterate over tabs and newlines")
unittest
{
	auto input = `5	1	9	5
7	5	3
2	4	6	8`;
	...
	assert(lines[0].splitter("\t").array.length == 4);
	assert(lines[1].splitter("\t").array.length == 3);
	assert(lines[2].splitter("\t").array.length == 4);
}

@("We can parse lists of integers")
unittest
{
	import std.conv : to, parse;
	import std.algorithm : splitter, map;
	import std.range : array;
	auto input = "1	2	54	78";
	auto values = input.splitter("\t");
	auto integers = values.map!(v => v.parse!size_t).array;
	assert(integers == [1, 2, 54, 78]);
}

size_t checksumLine(string line)
{
	import std.conv : to, parse;
	import std.algorithm : splitter, map, minElement, maxElement;

	auto values = line.splitter("\t");
	debug writeln("Values" ~ values.to!string);

	auto integers = values.map!(v => v.parse!size_t);
	size_t smallest = integers.minElement;
	size_t largest = integers.maxElement;

	debug writeln("Largest: " ~ largest.to!string ~ " Smallest: " ~ smallest.to!string ~ " Difference: " ~ (largest - smallest).to!string);
...
@("We can checksum a line")
unittest
{
	auto input = "100	1	6	35";

	assert(checksumLine(input) == 99);
	assert(checksum(input) == 99);
}